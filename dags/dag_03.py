...
with DAG(
    ...
    create_yellow_table = BigQueryCreateTableOperator(
        task_id="create_yellow_table",
    ...
    # Can replace with BigQueryInsertJobOperator
    create_yellow_table_ex = BigQueryCreateTableOperator(
        task_id="create_yellow_table_ext",
        dataset_id=DATASET_ID,
        gcp_conn_id="gcp_connection",
            ...
            "query": {
                "query": f"""
                    ...
                    SELECT
                        ...
                        COALESCE(CAST({"tpep_pickup_datetime" if taxi_type == "yellow" else "lpep_pickup_datetime"} AS STRING), ""),
                        COALESCE(CAST({"tpep_dropoff_datetime" if taxi_type == "yellow" else "lpep_dropoff_datetime"} AS STRING), ""),
    ...
    # )
    create_green_table = BigQueryInsertJobOperator(
        task_id="create_green_table",
        configuration={
            "query": {
                "query": f"""
    ...
    create_green_table_ex = BigQueryInsertJobOperator(
        task_id="create_green_table_ext",
        configuration={
            "query": {
                "query": f"""
                    CREATE OR REPLACE EXTERNAL TABLE `{DATASET_ID}.{table_staging}_ext`
                    (
                        VendorID STRING OPTIONS (description = 'A code indicating the LPEP provider that provided the record. 1= Creative Mobile Technologies, LLC; 2= VeriFone Inc.'),
                        lpep_pickup_datetime TIMESTAMP OPTIONS (description = 'The date and time when the meter was engaged'),
                        lpep_dropoff_datetime TIMESTAMP OPTIONS (description = 'The date and time when the meter was disengaged'),
    ...
    bq_green_merge_task = BigQueryInsertJobOperator(
    ...
    download_file_and_unzip >> upload_to_gcs_and_remove_local >> taxi_type_branch
    taxi_type_branch >> [yellow_taxi_branch, green_taxi_branch, empty_branch]
    (
        yellow_taxi_branch
        >> create_yellow_table
        >> create_yellow_table_ex
        >> bq_create_temp_table
        >> bq_merge_task
    )
    (
        green_taxi_branch
        >> create_green_table
        >> create_green_table_ex
        >> bq_create_temp_table
        >> bq_green_merge_task
    )
